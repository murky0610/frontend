...
export default function FilterDropdown() {
  ...
  const [focuses, setFocuses] = React.useState<string[]>([])
    ...
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <Button variant="outline">
          ...
          Filter
          {(categories.length > 0 || focuses.length > 0 || commodities.length > 0 || startDate || endDate || authors.length > 0) && (
            <span className="ml-2">
              ({[
                categories.length,
                focuses.length,
      ...
      <DropdownMenuContent className="w-80 p-4 space-y-4">
          ...
          <div className="space-y-2">
            {Object.entries(commodityIcons).map(([key, IconComponent]) => (
              <div key={key} className="flex items-center gap-2">
                ...
                />
                <label htmlFor={`commodity-${key}`} className="flex items-center gap-2">
                  <IconComponent className="w-4 h-4" />
                  <span>{key}</span>
                </label>
              </div>
        ...
        <div className="flex gap-2 mt-4">
          <Button 
            ...
            className="flex-1"
            disabled={categories.length === 0 && focuses.length === 0 && commodities.length === 0 && !startDate && !endDate && authors.length === 0}
          >
            Clear
          </Button>
          <Button 
